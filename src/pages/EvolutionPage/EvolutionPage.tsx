...
const EvolutionPage = () => {
  ...
  const processEvolutionChain = async (chain) => {
    ...
    const extractEvolutions = async (current, level = 0, parent = null) => {
      ...
      const numberMatch = speciesUrl.match(/\/pokemon-species\/(\d+)\//);
      ...
      if (pokemonId) {
        try {
          ...
          console.error(
            `Error fetching details for Pokémon ${pokemonId}:`,
            error
          );
        }
      }
  ...
  const getEvolutionCondition = (details) => {
    ...
    if (details.min_happiness) {
      return {
        type: "happiness",
        description: `Happiness ≥ ${details.min_happiness}`,
      };
    }
    ...
    if (details.time_of_day) {
      return {
        type: "time",
        description: `Level up during ${details.time_of_day}`,
      };
    }
    ...
    if (details.known_move) {
      return {
        type: "move",
        description: `Knows move ${details.known_move.name.replace("-", " ")}`,
      };
    }
    ...
    if (details.min_affection) {
      return {
        type: "affection",
        description: `Affection ≥ ${details.min_affection}`,
      };
    }
    ...
    if (details.location) {
      return {
        type: "location",
        description: `Level up at ${details.location.name.replace("-", " ")}`,
      };
    }
              ...
              {evolution.level > 0 && (
                <div className={styles.evolutionCondition}>
                  <span className={styles.conditionLabel}>
                    {t("evolution.condition")}:
                  </span>
                  <span className={styles.conditionValue}>
                    {evolution.evolution_condition.description}
                  </span>
                </div>
              )}
        ...
        <div className={styles.statsComparison}>
          ...
          <div className={styles.statsTable}>
              ...
              {evolutionChain.map((evo) => {
                ...
                return (
                  <div key={evo.id} className={styles.statValueCell}>
                    <div
                      className={styles.statValueBar}
                      style={{ width: `${(hpStat?.value / 255) * 100}%` }}
                    >
                      {hpStat?.value || "?"}
                    </div>
                  </div>
              ...
              {evolutionChain.map((evo) => {
                ...
                return (
                  <div key={evo.id} className={styles.statValueCell}>
                    <div
                      className={styles.statValueBar}
                      style={{ width: `${(attackStat?.value / 255) * 100}%` }}
                    >
                      {attackStat?.value || "?"}
                    </div>
                  </div>
              ...
              {evolutionChain.map((evo) => {
                ...
                return (
                  <div key={evo.id} className={styles.statValueCell}>
                    <div
                      className={styles.statValueBar}
                      style={{ width: `${(defenseStat?.value / 255) * 100}%` }}
                    >
                      {defenseStat?.value || "?"}
                    </div>
                  </div>
            ...
            <div className={styles.statsRow}>
              ...
              {evolutionChain.map((evo) => {
                const spAtkStat = evo.stats.find(
                ...
                return (
                  <div key={evo.id} className={styles.statValueCell}>
                    <div
                      className={styles.statValueBar}
                      style={{ width: `${(spAtkStat?.value / 255) * 100}%` }}
                    >
                      {spAtkStat?.value || "?"}
                    </div>
                  </div>
            ...
            <div className={styles.statsRow}>
              ...
              {evolutionChain.map((evo) => {
                const spDefStat = evo.stats.find(
                ...
                return (
                  <div key={evo.id} className={styles.statValueCell}>
                    <div
                      className={styles.statValueBar}
                      style={{ width: `${(spDefStat?.value / 255) * 100}%` }}
                    >
                      {spDefStat?.value || "?"}
                    </div>
                  </div>
              ...
              {evolutionChain.map((evo) => {
                ...
                return (
                  <div key={evo.id} className={styles.statValueCell}>
                    <div
                      className={styles.statValueBar}
                      style={{ width: `${(speedStat?.value / 255) * 100}%` }}
                    >
                      {speedStat?.value || "?"}
                    </div>
                  </div>
