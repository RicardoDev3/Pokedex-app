...
const RegionMapPage = () => {
  ...
  const fetchPokemonLocations = async (id) => {
    try {
        ...
        locationsData.map(async (locationData) => {
          ...
          return {
            ...
            region: {
              ...
              displayName: regionDetail.names.find(n => n.language.name === 'en')?.name || regionDetail.name
            },
      ...
      const uniqueRegions = Array.from(
        ...
        const location = detailedLocations.find(loc => loc.region.id === regionId);
        return {
          id: regionId,
  ...
  const renderRegionMap = () => {
    ...
    const mapImage = `/maps/${region.name}.png`;
    
    return (
      <div className={styles.mapContainer}>
        <div className={styles.mapHeader}>
          <h2 className={styles.mapTitle}>{region.displayName} {t('region.region')}</h2>
          ...
          {locations.length > 0 ? (
            <>
              <div className={styles.regionsTabContainer}>
                <div className={styles.regionsTab}>
                  {regions.map(region => (
                    <button 
                      key={region.id}
                      className={`${styles.regionButton} ${selectedRegion === region.name ? styles.active : ''}`}
              ...
              <div className={styles.locationsList}>
                <h3 className={styles.locationsTitle}>{t('region.detailedLocations')}</h3>
                
                <div className={styles.locationsGrid}>
                  {displayedLocations.map(location => (
                    <div key={location.id} className={styles.locationCard}>
                      <h4 className={styles.locationCardTitle}>{location.location}</h4>
                      <div className={styles.locationAreaName}>
                        {location.name}
                      </div>
                      <div className={styles.locationDetails}>
                        <div className={styles.detailItem}>
                          <span className={styles.detailLabel}>{t('region.region')}:</span>
                          <span className={styles.detailValue}>{location.region.displayName}</span>
                        </div>
                        <div className={styles.detailItem}>
                          <span className={styles.detailLabel}>{t('region.levels')}:</span>
                          <span className={styles.detailValue}>{getMinMaxLevels(location)}</span>
                        </div>
                        <div className={styles.detailItem}>
                          <span className={styles.detailLabel}>{t('region.games')}:</span>
                          <span className={styles.detailValue}>
                            {location.encounter_details.map(detail => 
                ...
                {filteredLocations.length > 8 && !showAllLocations && (
                  ...
                  >
                    {t('region.showMore', { count: filteredLocations.length - 8 })}
                  </button>
                )}
